...
fun Node.getChildren(tag: String): Node? {
    val childrenNodes = childNodes ?: return null
    for (i in 0..childrenNodes.length -1) {
        val item = childrenNodes.item(i)
        if (item != null) {
...
fun Node.getText(tag: String): String {
    val n = getChildren(tag)
    if (n != null) {
        var text = n.firstChild
        if (text != null) {
